...
class Head(nn.Module):
    ...
    def forward(self, x):
        ...
        attention_score = q_t @ k_t.transpose(-2, -1)
        dk = q_t.size(-1)
        attention_score = attention_score / (dk ** 0.5)
        attention_score = attention_score.masked_fill(self.tril[:T, :T] == 0, float('-inf'))
        out = F.softmax(attention_score ,  dim=-1)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # Your implementation here
        attentions = [h(x) for h in self.heads]
...
class BigramLanguageModel(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        position_embeddings = self.position_embedding_table(torch.arange(T, device=device)).unsqueeze(0)
        ...
        # Your implementation here
        x = self.blocks(combined_embeddings)
        x = self.ln_f(x)
